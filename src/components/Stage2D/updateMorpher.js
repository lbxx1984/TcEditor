...
export default function updateMorpher(nextProps, me) {
    ...
    if (nextProps.tool === 'pickJoint' && me.props.tool !== 'pickJoint' && nextProps.selectedMesh) {
        me.morpher2D.attach(nextProps.selectedMesh);
    }
    if (nextProps.tool === 'pickJoint' && nextProps.selectedMesh !== me.props.selectedMesh) {
        me.morpher2D.attach(nextProps.selectedMesh);
        me.morpher2D.attachAnchor(null);
    }
    if (nextProps.tool === 'pickJoint' && nextProps.selectedVectorIndex !== me.props.selectedVectorIndex) {
        me.morpher2D.attachAnchor(nextProps.selectedVectorIndex);
    }
    if (nextProps.tool === 'pickJoint' && nextProps.timer !== me.props.timer && me.morpher2D.mesh) {
